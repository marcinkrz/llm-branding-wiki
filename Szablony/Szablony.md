---
typ: indeks
aktualizacja: 2026-08-07
---

# Szablony

*Artefakty do wypełnienia podczas pracy z procesem. Każdy szablon odpowiada konkretnemu wynikowi etapu ([[Playbooki/00 Jak korzystać z procesu|playbooki]]). Kopiuj do notatki projektu i wypełniaj.*

## Szablony

| Szablon | Etap | Co daje |
|---|---|---|
| [[Szablony/Kwestionariusz discovery\|Kwestionariusz discovery]] | 01 Odkrywanie | Pytania do klienta na start (ma klientów) |
| [[Szablony/Kwestionariusz discovery nowy biznes\|Kwestionariusz — nowy biznes]] | 01 Odkrywanie | Pytania gdy klient nie ma jeszcze klientów |
| [[Szablony/Struktura projektu\|Struktura projektu]] | cały os (01+) | Standard folderu projektu + index.html + DESIGN.md |
| [[Szablony/Warsztat strategiczny\|Warsztat strategiczny]] | 01/06/09 | Agenda warsztatu z klientem |
| [[Szablony/Brief kreatywny\|Brief kreatywny]] | 01/09/10 | Punkt wyjścia dla projektanta |
| [[Szablony/Persona klienta\|Persona klienta]] | 03 Klient | Profil segmentu |
| [[Szablony/Oświadczenie pozycjonowania\|Oświadczenie pozycjonowania]] | 06 Pozycjonowanie | Formuła pozycji |
| [[Szablony/StoryBrand\|StoryBrand]] | 07 Komunikacja | Struktura przekazu |
| [[Szablony/Oferta\|Oferta]] | 08 Oferta | Struktura oferty |
| [[Szablony/Moodboard kierunek wizualny\|Moodboard / kierunek wizualny]] | 09 Tożsamość | Kierunek ID przed projektem |
| [[Szablony/Walidacja nazwy i logo\|Walidacja nazwy i logo]] | 09 Tożsamość | Testy nazwy/logo przed rebrandingiem |
| [[Szablony/Struktura strony\|Struktura strony]] | 10 Strona | Wireframe sekcji |
| [[Szablony/Plan contentu\|Plan contentu]] | 11 Treść | Kalendarz tematów |

## Jak używać
1. Znajdź etap w [[Playbooki/00 Jak korzystać z procesu|playbookach]].
2. Skopiuj właściwy szablon do notatki projektu (`00 Projekty/<nazwa>/`).
3. Wypełniaj danymi z researchu (nie wyobraźnią).

## Zobacz też
- [[Frameworki/00 Szablon frameworka]] — szablon dla nowych frameworków.
- [[Checklisty/Checklisty|Checklisty]] — weryfikacja wypełnionych artefaktów.
