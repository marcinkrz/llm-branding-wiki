---
typ: szablon
użycie: karta frameworka
aktualizacja: 2026-08-07
---

# [NAZWA FRAMEWORKA]

> **Pytanie przewodnie:** *Jak wykorzystać to podczas tworzenia strategii?*

## Co rozwiązuje

(Jaki problem strategiczny ten framework rozwiązuje. Dlaczego istnieje.)

## Jakie ma ograniczenia

(Czego nie potrafi. Gdzie zawodzi. Jakie założenia przyjmuje.)

## Kiedy warto ją stosować

(Sytuacje, w których framework daje najlepsze wyniki. Jakie warunki muszą być spełnione.)

## Kiedy lepiej wybrać inne podejście

(Kiedy NIE używać. Sygnały ostrzegawcze, alternatywy.)

## Z czym dobrze się łączy

(Inne frameworki z biblioteki, z którymi działa synergicznie. Wstaw linki typu: [[Frameworki/01 StoryBrand|StoryBrand]].)

## Jakie decyzje pomaga podjąć

(Konkretne decyzje strategiczne, które ten framework umożliwia.)

## Jakie dane wejściowe są potrzebne

(Date do zebrania przed użyciem. Co muszę wiedzieć.)

## Jaki artefakt jest wynikiem

(Wynik: dokument, tabela, canvas, oświadczenie pozycjonowania itp.)

## Jak zweryfikować jakość wyniku

(Kryteria, testy, pytania kontrolne. Skąd wiem, że zrobiłem to dobrze.)

---

## Typowe błędy początkujących

*(Wypisz 2–4 najczęstsze błędy — np.:)*
- Np.: traktowanie frameworka jako „odpowiedzi" zamiast narzędzia do myślenia.
- Np.: użycie bez danych wejściowych (wypełnianie intuicją zamiast badaniem).
- Np.: pominięcie sekcji „kiedy NIE używać".

## Pytania do klienta

*(Wypisz 3–5 pytań do klienta, które ten framework pomaga zadać — np.:)*
- Np.: „Kiedy ostatnio stanąłeś przed tym problemem — co zrobiłeś dokładnie?"
